...
async function saveDiscord(user) {
  ...
  const { error } = await supabase.from("linked_accounts").upsert(
    { discord_id: user.id, discord_username: user.username, discord_global_name: user.global_name, discord_avatar: user.avatar, updated_at: new Date().toISOString() },
...
async function assignVerifiedRole(discordId) {
  ...
  await fetch(`https://discord.com/api/v10/guilds/${guildId}/members/${discordId}/roles/${roleId}`, { method: "PUT", headers: { Authorization: `Bot ${token}` } });
}
...
export async function GET(request) {
  ...
  try {
    if (code && process.env.DISCORD_CLIENT_ID && process.env.DISCORD_CLIENT_SECRET) {
      const tokenRes = await fetch("https://discord.com/api/oauth2/token", {
        ...
        body: new URLSearchParams({ client_id: process.env.DISCORD_CLIENT_ID, client_secret: process.env.DISCORD_CLIENT_SECRET, grant_type: "authorization_code", code, redirect_uri: DISCORD_CALLBACK_URL })
      });
  ...
  } catch (err) { console.error("[discord] save/link failed", err); }
